 ...
 */
export function resolveSelector(el: Element): Selector {
  // P1: data-testid
  ...
  if (role) {
    ...
    if (labelledBy) {
      ...
      if (labelEl?.textContent?.trim()) return { role, name: labelEl.textContent.trim() };
    }
  }
  ...
  if (id) {
    ...
    if (labelEl?.textContent?.trim()) return { label: labelEl.textContent.trim() };
  }
