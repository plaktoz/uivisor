...
const capture = vi.fn();
beforeAll(() => {
    window.__uivisorCapture = capture;
...
describe('CAPTURE_SCRIPT', () => {
    ...
    it('CS-03: CAPTURE_SCRIPT re-exported from index.ts resolves as a string', async () => {
        ...
        expect(typeof mod.CAPTURE_SCRIPT).toBe('string');
    });
    ...
    it('CS-22: window.__uivisorCapture undefined -> click does not throw; no command emitted', () => {
        const saved = window.__uivisorCapture;
        delete window.__uivisorCapture;
        const btn = document.createElement('button');
        btn.textContent = 'Test';
        document.body.appendChild(btn);
        expect(() => btn.click()).not.toThrow();
        expect(capture).not.toHaveBeenCalled();
        window.__uivisorCapture = saved;
    });
});
//# sourceMappingURL=captureScript.test.js.map